...
export class HousingService {
  ...
  getAllHousingLocations(): Observable<HousingLocation[]> {
    return this.http
      ...
      .pipe(
        catchError(
          this.handleError<HousingLocation[]>("getAllHousingLocations", [])
        )
      );
  }
  ...
  getLoationNo404(id: number): Observable<HousingLocation> {
    ...
    return this.http.get<HousingLocation[]>(url).pipe(
      ...
      catchError(
        this.handleError<HousingLocation>(`getHousingLocation id=${id}`)
      )
    );
  }
  ...
  getHousingLocationById(id: number): Observable<HousingLocation | undefined> {
    return this.http
      ...
      .pipe(
        catchError(
          this.handleError<HousingLocation>(`getHousingLocationById id=${id}`)
        )
      );
  }
